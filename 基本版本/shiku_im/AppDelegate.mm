...
//    [NSThread sleepForTimeInterval:0.3];
    ...
    [JPUSHService setupWithOption:launchOptions appKey:@"691fb017dd9b422fd16cb1db"
                          channel:@"App Store"
...
//    [g_notify postNotificationName:kDidReceiveRemoteNotification object:pushNotificationKey];
    ...
    // 设置主题颜色
    [[SkinManage sharedInstance] switchSkinIndex:8];
...
- (void)setUserAgent {
    ...
    WKWebView *webView = [[WKWebView alloc] init];
     ...
     [webView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id result, NSError *error) {
         ...
         NSString *newUA = [NSString stringWithFormat:@"%@ %@",result,@"app-shikuimapp"];
...
- (void) showAlert: (NSString *) message completeBlock:(void(^)())completeBlock {
    UIAlertView *av = [[UIAlertView alloc] initWithTitle:APP_NAME message:message delegate:self cancelButtonTitle:Localized(@"JX_Confirm") otherButtonTitles:nil, nil];
...
- (CXProviderConfiguration *)providerConfig{
    ...
    dispatch_once(&onceToken, ^{
        ...
        UIImage* iconMaskImage = [UIImage imageNamed:@"app_icon"];
