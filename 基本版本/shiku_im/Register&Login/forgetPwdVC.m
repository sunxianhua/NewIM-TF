...
- (void)viewDidLoad
{
   ...
   _btn.frame = CGRectMake(INSETS, n, WIDTH, 56);
...
-(void)onClick:(UIButton *)btn{
   ...
   if (self.isModify){
      ...
      if(b){
         [_wait start];
         [g_server resetPwd:_phone.text areaCode:areaCode randcode:_smsCode newPwd:_pwd.text passtype:self.passtype toView:self];
...
//      if (self.passtype) {
//         [g_server sendSMS:[NSString stringWithFormat:@"%@",_phone.text] areaCode:areaCode isRegister:YES imgCode:_imgCode.text toView:self];
//      }else{
