...
//        [self.tableBody addSubview:grayLine];
        ...
        iv.frame = CGRectMake(0, 0, JX_SCREEN_WIDTH, 200.0);
        h = CGRectGetMaxY(iv.frame)-100.0;
        
        UIView *witeView = [UIView new];
        witeView.backgroundColor = [UIColor whiteColor];
        [self.tableBody addSubview:witeView];
        witeView.frame = CGRectMake(MY_INSET,h, JX_SCREEN_WIDTH, 50);
        witeView.layer.cornerRadius = 25.0;
        witeView.layer.masksToBounds = YES;
        h = CGRectGetMaxY(witeView.frame) - 30.0;
...
//        iv = [self createButton:Localized(@"JX_MyBalance") drawTop:NO drawBottom:YES icon:THESIMPLESTYLE ? @"balance_recharge_icon" : @"balance_recharge_icon" click:@selector(onRecharge)];
...
//            [self.tableBody addSubview:line];
        ...
        iv = [self createButton:Localized(@"JX_MyDynamics") drawTop:NO drawBottom:YES icon:THESIMPLESTYLE ? @"my_space_simple" : @"矢量智能对象_86" click:@selector(onMyBlog)];
        iv.frame = CGRectMake(MY_INSET,h, w-MY_INSET*2, HEIGHT);
        h+=iv.frame.size.height;
        
        iv = [self createButton:Localized(@"JX_MyCollection") drawTop:NO drawBottom:YES icon:THESIMPLESTYLE ? @"collection_me_simple" : @"select拷贝" click:@selector(onMyFavorite)];
        iv.frame = CGRectMake(MY_INSET,h, w-MY_INSET*2, HEIGHT);
        h+=iv.frame.size.height;
...
//        h+=iv.frame.size.height;
        ...
        iv = [self createButton:Localized(@"JX_MyLecture") drawTop:NO drawBottom:YES icon:THESIMPLESTYLE ? @"my_lecture_simple" : @"kecheng_icon" click:@selector(onCourse)];
        iv.frame = CGRectMake(MY_INSET,h, w-MY_INSET*2, HEIGHT);
        h+=iv.frame.size.height;
...
//        h+=iv.frame.size.height;
        ...
        iv = [self createButton:Localized(@"JXSettingVC_Set") drawTop:YES drawBottom:YES icon:THESIMPLESTYLE ? @"setting_icon" : @"setting_icon" click:@selector(onSetting)];
        iv.frame = CGRectMake(MY_INSET,h, w-MY_INSET*2, HEIGHT);
//        h+=iv.frame.size.height;
...
//    [btn addSubview:title];
    ...
    whiteBgView.frame = CGRectMake(0, CGRectGetMaxY(title.frame), JX_SCREEN_WIDTH, 40);
//    [btn addSubview:whiteBgView];
    
    UIView *baseView = [[UIView alloc] initWithFrame:CGRectMake(6, CGRectGetMaxY(title.frame)+ (THE_DEVICE_HAVE_HEAD ? 24 : 24), JX_SCREEN_WIDTH-12, 80.0)];
    baseView.layer.masksToBounds = YES;
    baseView.layer.cornerRadius = 10.f;
    baseView.backgroundColor = [UIColor clearColor];
    [btn addSubview:baseView];
    
    baseView.frame = CGRectMake(10, 0.0, JX_SCREEN_WIDTH - 20, 100.0);
    
//    // 头像阴影
//    JXImageView *shadow = [[JXImageView alloc]initWithFrame:CGRectMake(13, (baseView.frame.size.height-70)/2-5, 90, 90.0)];
...
//    [baseView addSubview:shadow];
    ...
    //头像
    _head = [[JXImageView alloc]initWithFrame:CGRectMake(20, (baseView.frame.size.height-84)/2, 70.0, 70.0)];
    _head.layer.cornerRadius = _head.frame.size.width / 2;
    _head.layer.masksToBounds = YES;
    _head.didTouch = @selector(onResume);
    _head.delegate = self;
    _head.layer.borderWidth = 2.0;
    ...
    [p mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_offset(-10.0);
        make.height.mas_equalTo(20.0);
        make.left.equalTo(_head.mas_right).mas_offset(10.0);
    }];
    
    
    //电话Label
    p = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(p.frame), CGRectGetMaxY(p.frame)+5.0, 100, 16)];
    p.font = SYSFONT(12);
    ...
    UIImage *arrow = [UIImage imageNamed:@"箭头拷贝"];
    UIImageView* iv = [[UIImageView alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH-INSETS-40-MY_INSET, (baseView.frame.size.height-20)/2, arrow.size.width, arrow.size.height)];
    iv.image = arrow;
    ...
    qrImgV.image = [UIImage imageNamed:@"qrcode"];
    [baseView addSubview:qrImgV];
    [qrImgV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_head);
        make.size.mas_equalTo(qrImgV.image.size);
        make.right.equalTo(iv.mas_left).mas_offset(-10.0);
    }];

    
    btn.image = [UIImage imageNamed:@"组40"];
...
//    }
    ...
    if(click){
        UIImageView* iv;
        UIImage *arrow = [UIImage imageNamed:@"箭头"];
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH-INSETS-20-3-MY_INSET, 16, arrow.size.width, arrow.size.height)];
        iv.image = arrow;
        [btn addSubview:iv];
