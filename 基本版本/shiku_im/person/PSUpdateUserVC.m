...
- (void) createCustomView {
    ...
    if (!self.isRegister) {
        ...
        if ([self.user.setAccountCount integerValue] > 0) {
            ...
            iv = [self createButton:@"通讯号" drawTop:NO drawBottom:YES must:NO click:nil];
        }else {
            
            iv = [self createButton:@"通讯号" drawTop:NO drawBottom:YES must:NO click:@selector(onShikuNum)];
        }
