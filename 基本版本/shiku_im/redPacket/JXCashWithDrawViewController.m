...
@interface JXCashWithDrawViewController ()<UITextFieldDelegate,UIAlertViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.bgView.frame = CGRectMake(15, 20, bgWidth, 280.0);
...
-(UIView *)balanceView{
    if (!_balanceView) {
        ...
        _balanceView.frame = CGRectMake(0, CGRectGetMaxY(_inputView.frame), bgWidth, 200.0);
        ...
        _withdrawalsBtn = [UIFactory createButtonWithRect:CGRectZero title:@"申请提现" titleFont:g_factory.font17 titleColor:[UIColor whiteColor] normal:nil selected:nil selector:@selector(withdrawalsBtnAction:) target:self];
...
//        _aliwithdrawalsBtn = [UIFactory createButtonWithRect:CGRectZero title:@"提现到微信" titleFont:g_factory.font17 titleColor:[UIColor whiteColor] normal:nil selected:nil selector:@selector(withdrawalsBtnAction:) target:self];
//        _aliwithdrawalsBtn.tag = 1000;
...
//        UILabel *noticeLabel = [UIFactory createLabelWith:CGRectMake(drawMarginX, CGRectGetMaxY(_withdrawalsBtn.frame)+10+60, bgWidth-drawMarginX*2, 30) text:@"最低提现金额30元，否则无法提现" font:g_factory.font14 textColor:[UIColor lightGrayColor] backgroundColor:nil];
...
- (void)didVerifyPay:(NSString *)sender {
    ...
    if (!self.isBindAlipay){
        [g_App showAlert:@"暂未绑定支付宝账号，是否前往绑定？" delegate:self tag:1 onlyConfirm:NO];
        return;
    }
    
    //申请提现到 支付宝 账号
    [g_server transforAliAccontInfor:_countTextField.text toView:self];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex NS_DEPRECATED_IOS(2_0, 9_0){
    ...
    if (buttonIndex == 1){
        JXAddBankCardViewController *vc = [JXAddBankCardViewController new];
...
- (void)didServerResultSucces:(JXConnection *)aDownload dict:(NSDictionary *)dict array:(NSArray *)array1{
    ...
    if ([aDownload.action isEqualToString:act_getAlipayAccount]) {
        ...
        if (dict[@"card"]) {
            ...
            self.isBindAlipay = YES;
        }else{
            
            self.isBindAlipay = NO;
            [g_App showAlert:@"暂未绑定信息，是否前往绑定？" delegate:self tag:1 onlyConfirm:NO];
    ...
    if ([aDownload.action isEqualToString:act_transferToAli]) {
        ...
        [g_App showAlert:@"提现成功" completeBlock:^{
            [self actionQuit];
        }];
        NSLog(@"提现-----%@",dict);
    }
