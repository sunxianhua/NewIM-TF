...
- (void)viewDidLoad {
    ...
    if(_isRoom){
        ...
        self.orderView.frame = CGRectMake(CGRectGetMaxX(_luckyView.frame), 0, JX_SCREEN_WIDTH, self.tableBody.contentSize.height);
    }else{
        ...
        self.orderView.frame = CGRectMake(CGRectGetMaxX(_nomalView.frame), 0, JX_SCREEN_WIDTH, self.tableBody.contentSize.height);
...
-(JXTopSiftJobView *)topSiftView{
    if (!_topSiftView) {
        ...
        if (_isRoom) {
            ...
            itemsArray = [[NSArray alloc] initWithObjects:Localized(@"JX_LuckGift"),Localized(@"JX_MesGift"), nil];
        }else{
...
- (void)didVerifyPay:(NSString *)sender {
    ...
    [g_server sendRedPacketV1:[_moneyText doubleValue] type:(int)self.indexInt count:[_countText intValue] greetings:_greetText roomJid:self.roomJid roodId:self.roomId toUserId:self.toUserId time:time secret:secret toView:self];
